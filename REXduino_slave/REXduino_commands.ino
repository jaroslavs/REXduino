...
void commandO(byte pin, byte state, byte verbose) {
  ...
  {
    switch (state) {  //set the pin to the desired mode
      case 0:
        digitalWrite(pin, LOW);
        break;
      case 1:
...
void commandT(byte pin, byte verbose) {
  ...
  if (pin < 255)  //valid pin number
  {
    if (onewire_busBlockingLength[pin] > 0) {  //some device needs the bus to remain in HIGH state
      if (verbose) {
        Serial.println("1W busy!");
      }
      reportError(ERROR_ONEWIRE, ERROR_ONEWIRE_BUSYBUS);
    } else {  //1-Wire bus is available
